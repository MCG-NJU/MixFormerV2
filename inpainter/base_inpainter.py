...
class BaseInpainters:
	...
	def inpaint_efficient(self, frames, masks, num_tcb, num_tca, dilate_radius=15, ratio=1):
		...
		for f in tqdm(range(0, video_length, self.neighbor_stride), desc='Inpainting image'):
			...
			if tcb_imgs is not None:
				selected_imgs = torch.cat([selected_imgs, tcb_imgs], dim=1)
				selected_masks = torch.cat([selected_masks, tcb_masks], dim=1)
			# integrate ref frames
			selected_imgs = torch.cat([selected_imgs, imgs[:, ref_ids]], dim=1)
			selected_masks = torch.cat([selected_masks, masks[:, ref_ids]], dim=1)
			# pad after
			if tca_imgs is not None:
				selected_imgs = torch.cat([selected_imgs, tca_imgs], dim=1)
				selected_masks = torch.cat([selected_masks, tca_masks], dim=1)
	...
	def inpaint(self, frames, masks, dilate_radius=15, ratio=1):
		...
		inpainted_frames: numpy array, T, H, W, 3
		"""
		assert frames.shape[:3] == masks.shape, 'different size between frames and masks, {} and {}'.format(frames.shape[:3], masks.shape)
		assert ratio > 0 and ratio <= 1, 'ratio must in (0, 1]'
		...
		inpainted_splits = []
		for id_split in tqdm(id_splits, desc="Inpainting each split"):
...
def run_inpaint(vname: str, mask_path: str):
	save_path = 'inpaint_results'
	...
	# mask_path = "debug"
	masks = []
	mnames = os.listdir(mask_path)
	mnames.sort()
	for mp in mnames:
		m = Image.open(os.path.join(mask_path, mp))
	...
	checkpoint = '/data/songtianhui.sth/projects/E2FGVI/release_model/E2FGVI-HQ-CVPR22.pth'
	device = 'cuda'
	# 2/3: initialise inpainter
	base_inpainter = BaseInpainters(checkpoint, device)
	...
	# output_file = "inpaint_result.mp4"
	out_writer = cv2.VideoWriter(output_file.name, cv2.VideoWriter_fourcc(*"mp4v"), 30, frameSize=(inpainted_frames[0].shape[1], inpainted_frames[0].shape[0]))
	for ti, inpainted_frame in enumerate(inpainted_frames):
		out_writer.write(cv2.cvtColor(inpainted_frame, cv2.COLOR_RGB2BGR))
...
if __name__ == '__main__':
	...
	vname = "/data1/songtianhui.sth/perception_test/val_videos/video_3937.mp4"
	frames = []
	vidcap = cv2.VideoCapture(vname)
	success, image = vidcap.read()
	count = 0
	...
	mask_path = "/data/songtianhui.sth/projects/MixFormerV2/debug"
	masks = []
	mnames = os.listdir(mask_path)
	mnames.sort()
	for mp in mnames:
	...
	# 3/3: inpainting (frames: numpy array, T, H, W, 3; masks: numpy array, T, H, W)
	# ratio: (0, 1], ratio for down sample, default value is 1
	inpainted_frames = base_inpainter.inpaint(frames[:300], masks[:300], ratio=0.5)   # numpy array, T, H, W, 3
