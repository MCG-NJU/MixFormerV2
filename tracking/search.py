...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python lib/search/run_searching.py --script %s --config %s --save_dir %s --use_lmdb %d " \
                    "--script_prv %s --config_prv %s" \
                    % (args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv, args.config_prv)
    elif args.mode == "multiple":
        train_cmd = "python -m torch.distributed.launch --master_port %d --nproc_per_node %d lib/search/run_searching.py " \
